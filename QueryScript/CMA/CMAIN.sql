(SELECT ECM.VESSEL_NM_AN, 
 ...
 decode((select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE),'USLA1','USLAX','USLA2','USLAX','USLA3','USLAX','USLA4','USLAX','USLA5','USLAX','USLA6','USLAX','USOA1','USOAK',(select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE))AS POD, 
 ...
 (SELECT ECM.VESSEL_NM_AN, 
  ...
  decode(ECM.EQP_STATUS_CODE,'OB','2','IB','3',' ')AS EQP_STATUS_CODE, 
  decode(ECM.LADEN_INDICATOR_AN,'E','4','F','5',' ')AS LADEN_INDICATOR_AN, 
  nvl((SELECT TCG.BARGE_BKG FROM TMS_CNTR_GRP1 TCG WHERE TCG.CNTR_AN=ECM.CNTR_AN),'NOBOOKING') BOOKING_NO_AN,
  nvl(ECM.MASTER_BOL_AN,'NOBL') MASTER_BOL_AN,
  to_char(ECM.ACTIVITY_TM,'YYYYMMDDHH24MI')AS ACTIVITY_TM,
  (select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POL_CODE)AS POL,
  decode((select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE),'USLA1','USLAX','USLA2','USLAX','USLA3','USLAX','USLA4','USLAX','USLA5','USLAX','USLA6','USLAX','USOA1','USOAK',(select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE))AS POD, 
  ...
  substr(replace(nvl(ECM.VISIT_VSL_CALL_SIGN_C,''),'-',''),1,9) AS VISIT_VSL_CALL_SIGN_C, 
  ECM.CNTR_OPERATOR_CODE, 
  translate(nvl(nvl((SELECT a.SHPR_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN),(SELECT a.CSGN_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN)), 'NO'), '!@#$%^&*-+|\\/[]{}<>?', '                    ')AS SHIPPER, 
  ECM.CNTR_AN, 
  ECM.CONTAINER_TYPE_CODE, 
  nvl(substr((SELECT stragg(a.SEAL_NO_AN) from EDI_CNTR_MOVE_SEAL a WHERE a.SEAL_NO_AN IS NOT NULL AND a.CNTR_MOVE_ID=ECM.ID),1,15),'NOSEAL')AS SEAL,
  decode(ECM.EQP_STATUS_CODE,'OB','2','IB','3',' ')AS EQP_STATUS_CODE,
  ...
  (SELECT MAX(a.UNDG_AN) from EDI_CNTR_MOVE_HAZ a where a.CNTR_MOVE_ID=ECM.ID and a.IMCO_AN=(SELECT MAX(b.IMCO_AN) from EDI_CNTR_MOVE_HAZ b where b.CNTR_MOVE_ID=ECM.ID))AS UNDG, 
  decode(ECM.IN_TRANSPORT_MODE_CODE,'GATE','3','RAIL','2','BARGE','8','3')AS IN_TRANSPORT_MODE_CODE, 
   nvl((SELECT VSL_C FROM TMV_VESSEL_VISIT WHERE EXT_VOY_C=ECM.VOYAGE_AN AND VISIT_VSL_NAME_AN =ECM.VESSEL_NM_AN),'BARGE')AS INLAND_CARR_TP_MEAN_CODE, 
  decode(ECM.DAMAGE_SEGMENT_AN,null,' ',' ',' ','1') AS DAMAGE_SEGMENT_AN,
  nvl(ECM.AREA_C,'B5') AS AREA_C 
