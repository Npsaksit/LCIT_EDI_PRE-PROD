(SELECT ECM.VESSEL_NM_AN, 
 ...
 nvl(substr(replace((SELECT stragg(a.SEAL_NO_AN) from EDI_CNTR_MOVE_SEAL a WHERE a.SEAL_NO_AN IS NOT NULL AND a.CNTR_MOVE_ID=ECM.ID),'+',''),1,15),'NOSEAL')AS SEAL,
 ...
 decode(ECM.DST_CODE,'LICD','BKK10','SCT','BKK07','LCIT','LCB05','ECT','BKK08','LCD','LCB03','TIPS','LCB02','MON','BKK03','LCT','BKK12','BKK50')AS DST_CODE, 
 ...
 decode(ECM.OUT_TRANSPORT_MODE_CODE,'GATE','3','RAIL','2','3')AS OUT_TRANSPORT_MODE_CODE, 
 ...
 nvl(DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N)),ECM.AREA_C) AS AREA_C  
 FROM TMS_OWNER.EDI_CNTR_MOVE ECM 
 WHERE UPPER(ECM.CNTR_OPERATOR_CODE) LIKE UPPER('CMA') 
 ...
 AND (nvl(DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N)),ECM.AREA_C)) LIKE :BERTH 
 ...
 decode(ECM.EQP_STATUS_CODE,'OB','2','IB','3','3')AS EQP_STATUS_CODE, 
 decode(ECM.LADEN_INDICATOR_AN,'E','4','F','5',' ')AS LADEN_INDICATOR_AN, 
 nvl(decode(ECM.LADEN_INDICATOR_AN,'E',(SELECT T.BOOKING_AN FROM TMS_CNTR_GRP1 C, TMS_TRML_DOC_REF T WHERE C.terminal_doc_ref_an= t.terminal_doc_ref_an(+) and c.CNTR_AN=ECM.CNTR_AN)),'NOBOOKING') AS BOOKING_AN, 
  nvl(ECM.MASTER_BOL_AN,'NOBL') MASTER_BOL_AN, 
 to_char(ECM.ACTIVITY_TM,'YYYYMMDDHH24MI')AS ACTIVITY_TM, 
 (select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POL_CODE)AS POL, 
 (select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE)AS POD, 
 ...
 nvl(DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N)),ECM.AREA_C) AS AREA_C 
 FROM TMS_OWNER.EDI_CNTR_MOVE ECM 
 WHERE UPPER(ECM.CNTR_OPERATOR_CODE) LIKE UPPER('APL') 
 AND ECM.MOVE_TYPE_AN IN ('GO','RO') 
 AND ECM.OUT_TRANSPORT_MODE_CODE IN ('RAIL','GATE','LOLO') 
 AND ECM.LADEN_INDICATOR_AN = 'E'AND ECM.CREATE_TM between to_date(':START_DATE','DD-MON-YYYY HH24:MI') AND to_date(':END_DATE','DD-MON-YYYY HH24:MI') 
 ...
 nvl((SELECT MAX(a.IMCO_AN) from EDI_CNTR_MOVE_HAZ a where a.CNTR_MOVE_ID=ECM.ID),'NODG') AS IMCO,
 (SELECT MAX(a.UNDG_AN) from EDI_CNTR_MOVE_HAZ a where a.CNTR_MOVE_ID=ECM.ID and a.IMCO_AN=(SELECT MAX(b.IMCO_AN) from EDI_CNTR_MOVE_HAZ b where b.CNTR_MOVE_ID=ECM.ID))AS UNDG, 
 decode(ECM.OUT_TRANSPORT_MODE_CODE,'GATE','3','RAIL','2','BARGE','8','3') AS OUT_TRANSPORT_MODE_CODE, 
 ...
 FROM TMS_OWNER.EDI_CNTR_MOVE ECM, TMV_VESSEL_VISIT TV 
 WHERE  ECM.TRGT_VVD_N = TV.VESSEL_VISIT_C 
 AND UPPER(ECM.CNTR_OPERATOR_CODE) LIKE UPPER('CMA') 
