 ...
 nvl(ECM.VGM_GROSS_WEIGHT,'0')AS VGM_GROSS_WEIGHT
  FROM TMS_OWNER.EDI_CNTR_MOVE ECM  WHERE ECM.CNTR_OPERATOR_CODE IN ('PPP')  AND ECM.MOVE_TYPE_AN IN ('GO','RO')  AND ECM.OUT_TRANSPORT_MODE_CODE IN ('RAIL','GATE','LOLO')  AND ECM.CREATE_TM between to_date(':START_DATE','DD-MON-YYYY HH24:MI') AND to_date(':END_DATE','DD-MON-YYYY HH24:MI')    AND (DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',DECODE(ECM.SHIPPING_STATUS_CODE,'ST',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C =(SELECT CG.PREV_VVD_N FROM TMS_OWNER.TMV_CNTR_GRP1 CG  WHERE CG.CNTR_AN = ECM.CNTR_AN))),(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N))) LIKE :BERTH  )  union all  (SELECT  ECM.VESSEL_NM_AN,
 ECM.VOYAGE_AN,
 substr(replace(nvl(ECM.VISIT_VSL_CALL_SIGN_C,' '),'-',''),1,9) AS VISIT_VSL_CALL_SIGN_C,
 ECM.CNTR_OPERATOR_CODE,
 substr(translate(nvl(nvl((SELECT a.SHPR_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN),(SELECT a.CSGN_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN)), 'NO'), '!@#$%^&*-+|\/[]{}<>?', '                    '),1,35)AS SHIPPER,
 ...
 decode(ECM.AREA_C,'DP',(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C =(SELECT IN_VESSEL_VISIT_C FROM TMS_CNTR_GRP1 WHERE CNTR_AN=ECM.CNTR_AN AND ROWNUM=1)),ECM.AREA_C)AS AREA_C,
 nvl(ECM.VGM_GROSS_WEIGHT,'0')AS VGM_GROSS_WEIGHT
  FROM TMS_OWNER.EDI_CNTR_MOVE ECM  WHERE ECM.CNTR_OPERATOR_CODE IN ('PPP')  AND ECM.MOVE_TYPE_AN = 'LD'  AND ECM.OUT_TRANSPORT_MODE_CODE IN ('BARGE')  AND ECM.CREATE_TM between to_date(':START_DATE','DD-MON-YYYY HH24:MI') AND to_date(':END_DATE','DD-MON-YYYY HH24:MI')  AND (DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',DECODE(ECM.SHIPPING_STATUS_CODE,'ST',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C =(SELECT CG.PREV_VVD_N FROM TMS_OWNER.TMV_CNTR_GRP1 CG  WHERE CG.CNTR_AN = ECM.CNTR_AN))),(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N))) LIKE :BERTH  )
