(SELECT 
 ...
 substr(translate(nvl(nvl((SELECT a.CSGN_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN),(SELECT a.SHPR_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN)), 'NO'), '!@#$%^&*-+|\/[]{}<>?', '                    '),1,35) AS SHIPPER,
 ECM.CNTR_AN,
 ...
 decode((select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE),'USLA1','USLAX','USLA2','USLAX','USLA3','USLAX','USLA4','USLAX','USLA5','USLAX','USLA6','USLAX','USOA1','USOAK','KRINC','KRICH',(select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POD_CODE)) AS POD, 
 ...
 decode(ECM.IN_TRANSPORT_MODE_CODE,'GATE','3','RAIL','2','3') AS IN_TRANSPORT_MODE_CODE,
 ...
 WHERE UPPER(ECM.CNTR_OPERATOR_CODE) LIKE UPPER('CNC')
...
SELECT 
 ...
 nvl((SELECT MAX(a.IMCO_AN) from EDI_CNTR_MOVE_HAZ a where a.CNTR_MOVE_ID=ECM.ID),'NODG') AS IMCO,
 (SELECT MAX(a.UNDG_AN) from EDI_CNTR_MOVE_HAZ a where a.CNTR_MOVE_ID=ECM.ID and a.IMCO_AN=(SELECT MAX(b.IMCO_AN) from EDI_CNTR_MOVE_HAZ b where b.CNTR_MOVE_ID=ECM.ID)) AS UNDG,
 decode(ECM.IN_TRANSPORT_MODE_CODE,'GATE','3','RAIL','2','BARGE','8','8') AS IN_TRANSPORT_MODE_CODE,
 nvl((SELECT EXT_VSL_C FROM TMV_VESSEL_VISIT WHERE EXT_VOY_C=ECM.VOYAGE_AN AND VISIT_VSL_NAME_AN=ECM.VESSEL_NM_AN AND VISIT_VSL_CALL_SIGN_C=ECM.VISIT_VSL_CALL_SIGN_C),'BARGE') AS INLAND_CARR_TP_MEAN_CODE,
 decode(ECM.DAMAGE_SEGMENT_AN,null,' ',' ',' ','1') AS DAMAGE_SEGMENT_AN,
 nvl(ECM.AREA_C,'B5') AS AREA_C
 FROM TMS_OWNER.EDI_CNTR_MOVE ECM, TMV_VESSEL_VISIT TV
 WHERE  ECM.TRGT_VVD_N = TV.VESSEL_VISIT_C
 AND UPPER(ECM.CNTR_OPERATOR_CODE) LIKE UPPER('CNC')
