 ...
 substr(rpad(nvl((SELECT DAMAGE_CONDITION_C FROM TMS_DAMAGE_CONDITION WHERE DAMAGE_CONDITION_C=(SELECT DAMAGE_CONDITION_C FROM TMS_CNTR_DAMAGE WHERE CNTR_DAMAGE_ID = (SELECT MAX(CNTR_DAMAGE_ID) FROM TMS_CNTR_DAMAGE WHERE CNTR_AN = ECM.CNTR_AN))),' '),3,' '),1,3) DAMAGE_CONDITION_C,
 substr(rpad(nvl((SELECT DAMAGE_CONDITION_DS FROM TMS_DAMAGE_CONDITION WHERE DAMAGE_CONDITION_C=(SELECT DAMAGE_CONDITION_C FROM TMS_CNTR_DAMAGE WHERE CNTR_DAMAGE_ID = (SELECT MAX(CNTR_DAMAGE_ID) FROM TMS_CNTR_DAMAGE WHERE CNTR_AN = ECM.CNTR_AN))),' '),35,' '),1,35) DAMAGE_CONDITION_C,
 nvl(ECM.AREA_C,'B5') AS AREA_C
 FROM EDI_CNTR_MOVE ECM, TMV_VESSEL_VISIT TV
 WHERE  ECM.TRGT_VVD_N = TV.VESSEL_VISIT_C
 AND ECM.CNTR_OPERATOR_CODE LIKE ('CKL')
