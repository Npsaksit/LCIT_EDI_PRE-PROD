SELECT
 ...
 substr(translate(nvl((SELECT a.SHPR_N from TMS_CNTR_HIST a where a.CNTR_SEQ=ECM.CNTR_SEQ AND a.CNTR_AN=ECM.CNTR_AN),'NO'), '!@#$%^&*-+|\/[]{}<>?', '                    '),1,35) SHIPPER,
 ECM.CNTR_AN,
 ...
 substr(replace(replace(nvl(ECM.INLAND_CARR_TP_MEAN_CODE,' '),'&','AND'),'+',''),1,17) AS INLAND_CARR_TP_MEAN_CODE,
 substr(decode(ECM.DAMAGE_SEGMENT_AN,null,' ',' ',' ','1'),1,3) AS DAMAGE_SEGMENT_AN,
  nvl(ECM.AREA_C,'B5') AS AREA_C
 FROM EDI_CNTR_MOVE ECM, TMV_VESSEL_VISIT TV
 WHERE  ECM.TRGT_VVD_N = TV.VESSEL_VISIT_C
 AND ECM.CNTR_OPERATOR_CODE LIKE ('ANL') 
