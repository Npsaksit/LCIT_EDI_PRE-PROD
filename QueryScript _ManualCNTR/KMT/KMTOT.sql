 ...
 decode(ECM.EQP_STATUS_CODE,'OB','2','IB','3','TS','6','2')AS EQP_STATUS_CODE,
 decode(ECM.LADEN_INDICATOR_AN,'E','4','F','5',' ')AS LADEN_INDICATOR_AN,
 nvl(decode(ECM.LADEN_INDICATOR_AN,'E',decode(ECM.SHIPPING_STATUS_CODE,'BT',ECM.BOOKING_NO_AN,(SELECT TR.BOOKING_AN FROM TMS_TRML_DOC_REF TR WHERE TR.TERMINAL_DOC_REF_ID=(SELECT TERMINAL_DOC_REF_ID FROM TMS_TRML_DOC_REF_CNTR WHERE CNTR_CYCLE_ID=(SELECT CNTR_CYCLE_ID FROM TMS_CNTR_CYCLE WHERE DEPARTURE_CNTR_SEQ=ECM.CNTR_SEQ)))),ECM.BOOKING_NO_AN), 'NOBOOKING')AS BOOKING_NO_AN,
 ...
 decode(nvl((select tl.CTRY_C || tl.LOC_C from tmt_loc tl where tl.loc_c = ECM.POL_CODE), ' '),' ', ' ', nvl(ECM.DST_CODE,' ')) AS DST_CODE,
 ...
 WHERE ECM.CNTR_OPERATOR_CODE IN ('KMT')
 ...
 AND nvl((DECODE((SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N),'',DECODE(ECM.SHIPPING_STATUS_CODE,'ST',ECM.AREA_C,(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C =(SELECT CG.PREV_VVD_N FROM TMS_OWNER.TMV_CNTR_GRP1 CG  WHERE CG.CNTR_AN = ECM.CNTR_AN))),(SELECT AREA_C FROM TMV_VESSEL_VISIT WHERE VESSEL_VISIT_C = ECM.TRGT_VVD_N))),ECM.AREA_C) LIKE :BERTH
