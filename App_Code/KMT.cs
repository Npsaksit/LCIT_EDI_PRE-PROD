...
// -----------------For Content Query Data form EdiTB.Rows[checkdtb]["COLUMNS_NAME"]  -----------------------------
			...
			for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
			{
				...
				Console.WriteLine("TDT+20+"+EdiTB.Rows[checkdtb]["VOYAGE_AN"].ToString()+"+1+++++"+EdiTB.Rows[checkdtb]["VISIT_VSL_CALL_SIGN_C"].ToString()+":103'");
				countSegment++;

				Console.WriteLine("LOC+15+LCB05'");
				countSegment++;

				Console.WriteLine("LOC+9+THLCH:139:6'");
				countSegment++;

				Console.WriteLine("NAD+MS+LCIT'");
				countSegment++;
				...
				{
						Console.WriteLine("RFF+EP:"+EdiTB.Rows[checkdtb]["BOOKING_NO_AN"].ToString()+"'");
						countSegment++;
				}
					Console.WriteLine("DTM+178:"+EdiTB.Rows[checkdtb]["ACTIVITY_TM"].ToString()+":203'");
					countSegment++;
...
EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"LO++COARRI'\r\n";
			  ...
			  for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
			  {
				...
				Console.WriteLine("UNH+"+dt.ToString("yyyyMMddHH")+"+COARRI:D:95B:UN'");
				countSegment++;

			  	Console.WriteLine("BGM+46++9'");
				countSegment++;

			  	Console.WriteLine("TDT+20+"+EdiTB.Rows[checkdtb]["VOYAGE_AN"].ToString()+"+1+++++"+EdiTB.Rows[0]["VISIT_VSL_CALL_SIGN_C"].ToString()+":103::"+EdiTB.Rows[0]["VESSEL_NM_AN"].ToString()+"'");
				countSegment++;
				...
				Console.WriteLine("NAD+CF+"+EdiTB.Rows[checkdtb]["CNTR_OPERATOR_CODE"].ToString()+":160:166'");
				countSegment++;
	...
	else if(SenderType.ToString() == "FTP")
	{
		MoveFile(SaveEDIfile,SenderType.ToString());
			...
			for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
			{
				...
				Console.WriteLine("BGM+98+Discharge/Loading report+9'");
				countSegment++;
				...
				Console.WriteLine("LOC+11+THLCH:139:6+THLCHDL"+EdiTB.Rows[0]["AREA_C"].ToString()+":TER:ZZZ'");
				countSegment++;
	...
	else if(SenderType.ToString() == "FTP")
	{
		...
		public static void MoveFile(string PathFileMove, string sender)
		{
					if(sender.ToString() == "FTP")
					{
						...
						DirectoryInfo to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\KMT\\IO");
