...
//-------------- For Header Table Query data from : EdiTB.Rows[0]["COLUMNS_NAME"].ToString()-------------
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"'\r\n";
...
// -----------------For Content Query Data form EdiTB.Rows[checkdtb]["COLUMNS_NAME"]  -----------------------------
							...
							for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
							{
								...
								Console.WriteLine("UNH+DJS"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"+CODECO:D:95B:UN:ITG14'");
								countSegment++;

								Console.WriteLine("BGM+34+DJS"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"+9'");
								countSegment++;

								Console.WriteLine("TDT+20+"+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"+1++DJS:172:20+++:"+EdiTB.Rows[0]["VISIT_VSL_CALL_SIGN_C"].ToString()+":103::"+EdiTB.Rows[0]["VESSEL_NM_AN"].ToString()+"'");
								countSegment++;

								Console.WriteLine("NAD+CF+DJS'");
								countSegment++;

								Console.WriteLine("NAD+AG+DJS'");
								countSegment++;
								...
								{
								Console.WriteLine("REF+BN:"+EdiTB.Rows[checkdtb]["BOOKING_NO_AN"].ToString()+"'");
								countSegment++;
								}
								...
								Console.WriteLine("LOC+9+THLCH+:139:6'");
			                   	...
			                   	Console.WriteLine("LOC+165+THLCH:139:6+"+SenderID.ToString()+":TER:ZZZ'");
			                   	countSegment++;
								...
								Console.WriteLine("TDT+1++"+EdiTB.Rows[checkdtb]["IN_TRANSPORT_MODE_CODE"].ToString()+"+31++++"+EdiTB.Rows[checkdtb]["INLAND_CARR_CODE"].ToString()+":146:ZZZ'");
								countSegment++;
								...
								Console.WriteLine("UNT+"+countSegment.ToString()+"+DJS"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"'");
	...
	// ------------------------------------ CREATE FOOTER IN EDI FILE ----------------------------------------------------
				...
				Console.WriteLine("UNZ+"+EdiTB.Rows.Count+"+"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"'");
...
// ---------------------------------------------- END OF CREATE EDI FILE PROCESS --------------------------------------------
			...
			case "OT":
				...
				EDIHeader=	"UNB+UNOA:2+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"'\r\n";
				...
				for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
				{
					...
					Console.WriteLine("BGM+36+DJS"+String.Format("{0:D7}",Int32.Parse(dt_profile.Rows[0]["RUNNING_NUMBER"].ToString()))+"+9'");
					countSegment++;
					...
					Console.WriteLine("TDT+1++"+EdiTB.Rows[checkdtb]["OUT_TRANSPORT_MODE_CODE"].ToString()+"+31++++"+EdiTB.Rows[checkdtb]["INLAND_CARR_CODE"].ToString()+":146:ZZZ'");
					countSegment++;
	...
	//------------------------------------------ END OF CREATE EDI FILE PROCESS ---------------------------------------------------------
		...
		public static void MoveFile(string PathFileMove, string sender)
		{
					if(sender.ToString() == "FTP")
					{
						...
						DirectoryInfo to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\DJS\\IO");
