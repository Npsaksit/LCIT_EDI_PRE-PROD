...
//-------------- For Header Table Query data from : EdiTB.Rows[0]["COLUMNS_NAME"].ToString()-------------
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CODCOSI"+TerArea.ToString().Substring(1,1)+"++CODECO'\r\n"+
...
// -----------------For Content Query Data form EdiTB.Rows[checkdtb]["COLUMNS_NAME"]  -----------------------------
							...
							for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
							{
								...
								{
									Console.WriteLine("LOC+165+THLCH:139:6+LCH05:STO:184'");
									countSegment++;	
								}

								if(EdiTB.Rows[checkdtb]["AREA_C"].ToString() == "C3")
								{
									Console.WriteLine("LOC+165+THLCH:139:6+LCH09:STO:184'");
									countSegment++;	
								}
	...
	// ------------------------------------ CREATE FOOTER IN EDI FILE ----------------------------------------------------
				...
				Console.WriteLine("UNZ+1+CODCOSI"+TerArea.ToString().Substring(1,1)+"'");
...
// ---------------------------------------------- END OF CREATE EDI FILE PROCESS --------------------------------------------
			...
			case "OT":
				...
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CODCOSO"+TerArea.ToString().Substring(1,1)+"++CODECO'\r\n"+
							"UNH+"+dt.ToString("yyyyMMddHH")+"+CODECO:D:95B:UN'\r\n"+
				...
				if(EdiTB.Rows[checkdtb]["BOOKING_NO_AN"].ToString() != "NOBOOKING")
					...
					Console.WriteLine("LOC+8+"+EdiTB.Rows[checkdtb]["POL"].ToString()+"'");
					countSegment++;
		...
		countSegment++;
		Console.WriteLine("UNT+"+countSegment.ToString()+"+"+dt.ToString("yyyyMMddHH")+"'");
		Console.WriteLine("UNZ+1+CODCOSO"+TerArea.ToString().Substring(1,1)+"'");
	//------------------------------------------ END OF CREATE EDI FILE PROCESS ---------------------------------------------------------
		Console.SetOut (oldOut);
		writer.Close();
		ostrm.Close();
		...
		break;
			...
			case "LD":
EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CORCOSL"+TerArea.ToString().Substring(1,1)+"++COARRI'\r\n"+
			...
			"NAD+CA+COS:160:ZZZ'\r\n";
			  ...
			  for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
			  {
				...
				Console.WriteLine("DTM+404:"+EdiTB.Rows[checkdtb]["ACTIVITY_TM"].ToString()+":203'");
				countSegment++;
				...
				if(EdiTB.Rows[checkdtb]["AREA_C"].ToString() == "B5")
				{
					Console.WriteLine("LOC+165+THLCH:139:6+LCH05:TER:ZZZ'");
					countSegment++;	
				}

				if(EdiTB.Rows[checkdtb]["AREA_C"].ToString() == "C3")
				{
					Console.WriteLine("LOC+165+THLCH:139:6+LCH09:TER:ZZZ'");
					countSegment++;	
				}
				...
				countSegment++;
				Console.WriteLine("UNT+"+countSegment.ToString()+"+"+dt.ToString("yyyyMMddHH")+"'");
				Console.WriteLine("UNZ+1+"+"CORCOSL"+TerArea.ToString().Substring(1,1)+"'");
//------------------------------------------ END OF CREATE EDI FILE PROCESS ---------------------------------------------------------
	Console.SetOut (oldOut);
	writer.Close();
	ostrm.Close(); 
	countSegment =0;
			...
			case "DG":
				EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CORCOSD"+TerArea.ToString().Substring(1,1)+"++COARRI'\r\n"+
							"UNH+"+dt.ToString("yyyyMMddHH")+"+COARRI:D:95B:UN'\r\n"+
			...
			for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
			{
				...
				Console.WriteLine("DTM+403:"+EdiTB.Rows[checkdtb]["ACTIVITY_TM"].ToString()+":203'");
				countSegment++;
		...
		countSegment++;
		Console.WriteLine("UNT+"+countSegment.ToString()+"+"+dt.ToString("yyyyMMddHH")+"'");
		Console.WriteLine("UNZ+1+"+"CORCOSD"+TerArea.ToString().Substring(1,1)+"'");
		Console.SetOut (oldOut);
		writer.Close();
		ostrm.Close();
		countSegment =0;
	...
	public static void MoveFile(string PathFileMove, string sender)
		{
			...
			{
				DirectoryInfo from = new DirectoryInfo(@""+PathFileMove);
				DirectoryInfo to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\COS\\IO");
				foreach(FileInfo fi in from.GetFiles("*.EDI"))
				{
					 fi.MoveTo(Path.Combine(to.ToString(),fi.Name));
				}
