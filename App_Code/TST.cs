...
// ---------------------------------------------- END OF CREATE EDI FILE PROCESS --------------------------------------------
			...
			case "OT":
					...
					for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
					{
						 Console.WriteLine(EdiTB.Rows[checkdtb]["CNTR_AN"].ToString()+","+EdiTB.Rows[checkdtb]["CONTAINER_TYPE_CODE"].ToString()+","+EdiTB.Rows[checkdtb]["ACTIVITY_TM"].ToString()+","+EdiTB.Rows[checkdtb]["STATUS"].ToString()+","+EdiTB.Rows[checkdtb]["TERMIANL"].ToString()+","+EdiTB.Rows[checkdtb]["PORT"].ToString()+",,,,,,,"+EdiTB.Rows[checkdtb]["OWNER_STATUS"].ToString());
...
EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"LO++COARRI'\r\n"+
			  ...
			  "LOC+9+THLCH:139:6+THLCHDL"+EdiTB.Rows[0]["AREA_C"].ToString()+":TER:ZZZ'\r\n"+
			  "NAD+MS+LCIT'\r\n";
		...
		MoveFile(SaveEDIfile,SenderType.ToString());
			...
			case "DG":
				EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"DI++COARRI'\r\n"+
							...
							"LOC+11+THLCH:139:6+THLCHDL"+EdiTB.Rows[0]["AREA_C"].ToString()+":TER:ZZZ'\r\n"+
							"NAD+MS+LCIT'\r\n";
	...
	else if(SenderType.ToString() == "FTP")
	{
		...
		public static void MoveFile(string PathFileMove, string sender)
		{
					if(sender.ToString() == "FTP")
					{
						...
						DirectoryInfo to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\TST\\IO");
