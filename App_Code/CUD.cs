...
//-------------- For Header Table Query data from : EdiTB.Rows[0]["COLUMNS_NAME"].ToString()-------------
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CODECO'\r\n"+
							"UNH+"+dt.ToString("yyyyMMddHH")+"+CODECO:D:95B:UN:ITG13'\r\n"+
							"BGM+34+"+dt.ToString("yyyyMMddHH")+"+9'\r\n"+
							"TDT+20+"+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"+1++CUL:172:20+++:103'\r\n"+
							"LOC+9+THLCH:139:6'\r\n"+
							"NAD+CF+CUL'\r\n";
...
// ---------------------------------------------- END OF CREATE EDI FILE PROCESS --------------------------------------------
				...
				if(SenderType.ToString() == "EMAIL")
				{
					...
					MoveFile(SaveEDIfile,SenderType.ToString(),Move.ToString());
				}
			...
			case "OT":
				...
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"CODECO'\r\n"+
							"UNH+"+dt.ToString("yyyyMMddHH")+"+CODECO:D:95B:UN:ITG13'\r\n"+
							"BGM+35+"+dt.ToString("yyyyMMddHH")+"+9'\r\n"+
							"TDT+20+"+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"+1++CUL:172:20+++:103'\r\n"+
							"LOC+11+THLCH:139:6'\r\n"+
							"NAD+CF+CUL'\r\n";
...
EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"COARRI'\r\n"+
			  ...
			  "TDT+20++1+++++"+EdiTB.Rows[0]["VISIT_VSL_CALL_SIGN_C"].ToString()+":103::"+EdiTB.Rows[0]["VESSEL_NM_AN"].ToString()+"'\r\n"+
			  "LOC+9+THLCH:139:6+"+SenderID.ToString()+":TER:ZZZ'\r\n"+
			  "NAD+MS+LCIT'\r\n";
...
//------------------------------------------ END OF CREATE EDI FILE PROCESS ---------------------------------------------------------
	...
	else if(SenderType.ToString() == "FTP")
	{
			...
			case "DG":
				EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"COARRI'\r\n"+
							...
							"LOC+11+THLCH:139:6+"+SenderID.ToString()+":TER:ZZZ'\r\n"+
							"NAD+MS+LCIT'\r\n";
	...
	if(SenderType.ToString() == "EMAIL")
   {
	...
	else if(SenderType.ToString() == "FTP")
	{
		...
		public static void MoveFile(string PathFileMove, string sender, String cntrmove)
		{
					if(sender.ToString() == "FTP")
					{
						DirectoryInfo from = new DirectoryInfo(@""+PathFileMove);
						
						if(cntrmove == "IN" || cntrmove == "OT")
						{
							DirectoryInfo to = to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\CUD\\IO");
						...
						{
							DirectoryInfo to = to = new DirectoryInfo(@""+pathfile.FTP.ToString()+"\\CUD\\LD");
							foreach(FileInfo fi in from.GetFiles("*.EDI"))
							{
								 fi.MoveTo(Path.Combine(to.ToString(),fi.Name));
							}
