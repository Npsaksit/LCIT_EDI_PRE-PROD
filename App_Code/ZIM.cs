...
//-------------- For Header Table Query data from : EdiTB.Rows[0]["COLUMNS_NAME"].ToString()-------------
				EDIHeader=	"UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+dt.ToString("yyMMddHHmm")+"++CODECO'\r\n"+
...
// -----------------For Content Query Data form EdiTB.Rows[checkdtb]["COLUMNS_NAME"]  -----------------------------
							...
							for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
							{
								...
								Console.WriteLine("DTM+7:"+EdiTB.Rows[checkdtb]["ACTIVITY_TM"].ToString()+":303'");
								countSegment++;
	...
	// ------------------------------------ CREATE FOOTER IN EDI FILE ----------------------------------------------------
				...
				Console.WriteLine("UNZ+1+"+dt.ToString("yyMMddHHmm")+"'");
		...
		break;
			...
			case "LD":
				EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"LO++COARRI'\r\n"+
							  ...
							  "LOC+9+THLCH:139:6+LCIT:TER:ZZZ'\r\n"+
							  "NAD+MS+LCIT'\r\n"+
							  "NAD+CA+ZIM'\r\n";
							  ...
							  for(int checkdtb =0 ;checkdtb < EdiTB.Rows.Count; checkdtb++)
							  {
								...
								if(EdiTB.Rows[checkdtb]["BOOKING_NO_AN"].ToString() != "NOBOOKING")
								{
									Console.WriteLine("FTX+AAI+++"+EdiTB.Rows[checkdtb]["BOOKING_NO_AN"].ToString()+", "+MyVessel[0].ToString()+" "+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"'");
									countSegment++;
								}
								else
								{
									Console.WriteLine("FTX+AAI+++"+MyVessel[0].ToString()+" "+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"'");
									countSegment++;
								}
			...
			case "DG":
				EDIHeader = "UNB+UNOA:1+"+SenderID+"+"+ReceiveID+"+"+dt.ToString("yyMMdd")+":"+dt.ToString("HHmm")+"+"+"DI++COARRI'\r\n"+
							...
							"TDT+20+"+EdiTB.Rows[0]["VOYAGE_AN"].ToString()+"+1+++++"+EdiTB.Rows[0]["VISIT_VSL_CALL_SIGN_C"].ToString()+":103::"+EdiTB.Rows[0]["VESSEL_NM_AN"].ToString()+"'\r\n"+
							"LOC+11+THLCH:139:6+LCIT:TER:ZZZ'\r\n"+
							"NAD+MS+LCIT'\r\n"+
							"NAD+CA+ZIM'\r\n";
